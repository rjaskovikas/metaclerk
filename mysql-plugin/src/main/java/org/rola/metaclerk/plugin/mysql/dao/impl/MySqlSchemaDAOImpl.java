...
public class MySqlSchemaDAOImpl implements SchemaTableListDAO, TableColumnsDAO, AllSchemaObjectsDAO, DbPrivilegesDAO {

    JdbcConnector dbc = null;

    public MySqlSchemaDAOImpl(JdbcConnector dbc) {
        this.dbc = dbc;
    }
    ...
    void selectInformationSchema() {
        try {
            dbc.executeUpdate("use information_schema");
        } catch (SQLException ex) {
            throw new SQLRuntimeException("Can't use information_schema in MySql", ex);
        }
    }

    public TableList getSchemaTableList(String schema) {
        MySqlSchemaTableListFetcher f = new MySqlSchemaTableListFetcher(dbc, "BASE TABLE");
        return f.fetchTableList(schema);
    }

    ViewList getSchemaViewList(String schema) {
        MySqlSchemaTableListFetcher f = new MySqlSchemaTableListFetcher(dbc, "VIEW");
    ...
    public void fillTableColumnDescription(TableDescription tableDescription) {
        MySqlTableColumnsFetcher f = new MySqlTableColumnsFetcher(dbc);
    ...
    public PrivilegeList getDbUserPrivilegeList(String dbUserName) {
        MySqlDbPrivilegesFetcher f = new MySqlDbPrivilegesFetcher(dbc);
