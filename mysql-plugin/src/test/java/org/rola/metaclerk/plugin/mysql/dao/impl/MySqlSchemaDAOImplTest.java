...
public class MySqlSchemaDAOImplTest {

    private JdbcConnector dbc;
    private MySqlSchemaDAOImpl dao;
    ...
    private void initDao() {
        dao.initDAO("jdbc:h2:mem:MySqlTest;DB_CLOSE_DELAY=-1;MODE=MYSQL", "", "");
    }
    ...
    public void initializedDaoObject_closeDAO_closesJDBCConnection() throws Exception {
        ...
        dao.getSchemaTableList("MySqlSchema"); // should not throw an exception because jdbc connection is open
        dao.closeDAO();
        try {
            dao.getSchemaTableList("MySqlSchema"); // should throw an exception because jdbc connection is closed
        } catch (Exception ex) {
    ...
    public void normalDbInitializationSequence_initDab_SelectsInformationSchema() throws Exception {
        ...
        initDao();
        dbc.executeSelectStatement("select * from table_privileges");

        // information_schema was successfully selected if exception was not thrown
    }
    ...
    public void schemaTableList_getSchemaTableList_returnsTwoTable() throws Exception {
        initDao();
        createFakeTablesTableAndFillWithData();
        TableList lst = dao.getSchemaTableList("MySqlSchema");
    ...
    private void checkTableListResult(TableList lst) {
        ...
        assertEquals("mysqlschema", lst.get(0).getOwnerName());
        assertEquals("test2_table", lst.get(1).getName());
        assertEquals("mysqlschema", lst.get(1).getOwnerName());
    }
    ...
    public void schemaViewList_getSchemaViewList_returnsTwoViews() throws Exception {
        initDao();
        createFakeTablesTableAndFillWithData();
        ViewList lst = dao.getSchemaViewList("MySqlSchema");
    ...
    public void schemaPrivileges_getDbUserPrivilegeList_returnsTwoPrivileges() throws Exception {
        ...
        PrivilegeList lst = dao.getDbUserPrivilegeList("mysqlschema");
    ...
    public void schemaMetadata_getAllSchemaObjects_returnsFilledSchemaObject() throws Exception {
        ...
        DbSchema schema = dao.getAllSchemaObjects("mysqlschema");
    ...
    private void fillPrivilegesTableWithData() throws SQLException {
        String
                ...
                " values ('''mysqlschema''@''localhost''', 'mysqlschema', 'test1_table', 'SELECT', 'YES')";
        dbc.executeUpdate(query);
        query = " insert into table_privileges (grantee, table_schema, table_name, privilege_type, is_grantable)" +
                " values ('''mysqlschema''@%', 'mysqlschema', 'test1_table', 'UPDATE', 'NO')";
        dbc.executeUpdate(query);
        // creating one record that should not be selected
        query = " insert into table_privileges (grantee, table_schema, table_name, privilege_type, is_grantable)" +
                " values ('''mysqlschema-no''@%', 'mysqlschema', 'test1_table', 'UPDATE', 'NO')";
        dbc.executeUpdate(query);
    }
    ...
    private void fillTablesTableWithData(String object_type, String table_type) throws SQLException {
        String query = "insert into tables (table_name, table_schema, table_type) " +
                "values ('test1_" + object_type + "', 'mysqlschema', '" + table_type + "')";
        dbc.executeUpdate(query);
        query = "insert into tables (table_name, table_schema, table_type) " +
                "values ('test2_" + object_type + "', 'mysqlschema','" + table_type + "')";
        dbc.executeUpdate(query);

        // creating one record that should not be selected
        query = "insert into tables (table_name, table_schema, table_type) " +
                "values ('test3_" + object_type + "', 'mysqlschema','" + table_type + "-no" + "')";
        dbc.executeUpdate(query);
    }
    ...
    private void fillColumnsTableWithData(String object_type) throws SQLException {
        String
                ...
                "values ('mysqlschema', 'test1_" + object_type + "', 'column1', 1, 'def1'," +
        ...
        query = "" +
                ...
                "is_nullable, data_type, character_maximum_length, character_octet_length, numeric_precision," +
                "numeric_scale, datetime_precision) " +
                "values ('mysqlschema', 'test1_" + object_type + "', 'column2', 2, 'def2'," +
        ...
        query = "" +
                ...
                "is_nullable, data_type, character_maximum_length, character_octet_length, numeric_precision," +
                "numeric_scale, datetime_precision) " +
                "values ('mysqlschema', 'test1-no_" + object_type + "', 'column1', 1, 'def'," +
                "'NO', 'DECIMAL', null, null, 7, " +
                "2, null)";
        dbc.executeUpdate(query);
    }
